...
from wardrobe import login_required,color,random_name,get_temperature,get_look,item_check,allowed_file,outfit_add,filter_rows
...
def img_process(file):
    ...
    if allowed_file(filename):
        ...
        # Check if image dimensions meet the minimum requirements
        if width < 400 or height < 400:
            flash('Something wrong with image shape')
            return 0

        if img:
            filename = random_name()  # Create a random name for storing the file

            # Path where the image will be saved
            path = os.path.join(app.config['UPLOAD_FOLDER'], filename) 

            # Commenting out the background removal for now
            # img = Image.fromarray(np.uint8(removeBg(img)))  # Removing the background

            img.thumbnail((420, 420))  # Resize the image for thumbnail
            img.save(path, optimize=True, quality=30)  # Save the image with optimization

            # Optionally, you can extract the primary color here if needed
            # primaryColor = color(path)

            # Return the image path and a default color (FFFFFF as a placeholder)
            answ = {'path': path, 'color': 'FFFFFF'}
            return answ
    else:
        return 0
...
def home():
    ...
    if request.method == 'POST':
        ...
        weather_data, image_url = get_weather(city)

    return render_template('home.html', weather=weather_data, image_url=image_url)
